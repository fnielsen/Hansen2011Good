...
# -*- coding: utf-8 -*-
#
# $Id: Hansen2010Diffusion_streamtweetmatrix.py,v 1.5 2010/11/17 12:40:14 fn Exp $
...
from re        import compile, findall, search, sub, split, UNICODE, IGNORECASE
...
filename_valence = '/home/fnielsen/fnielsen/data/Nielsen2009Responsible_emotion.csv'
filename_stopwords = '/home/fnielsen/matlab/brede/data/stop_english1.txt'
...
querydb = {"delete": {"$exists": False}, "user": {"$exists": True}}
# querydb = {"delete": {"$exists": False}, "user": {"$exists": True}, "user.lang": "en", "englishness": {"$gt": 0}}
...
f.write("""id, retweet, url, mention, hashtag, followers, followees, length, lengthnourl, englishness, enlang, valence, arousal, normvalence, normarousal\n""")


tweets = []
n = 1
for tweetdb in tweetsdb.find(querydb):
    frows_valence.write(str(tweetdb.get('sentiment',0)) + '\n');
    frows.write(sub('\n', ' ', str(tweetdb['id'])) + '\n');
    frows_date.write(sub('\n', ' ', str(tweetdb['created_at'])) + '\n');
    frows_user.write(sub('\n', ' ', str(tweetdb['user']['screen_name'])) + '\n');
    t = sub('\n', ' ', str(tweetdb['text']))
    frows_tweets.write(sub('\r', ' ', t) + '\n');
    if pattern_retweet.search(str(tweetdb['text'])):
        frows_retweet.write('1\n')
    else:
        frows_retweet.write('0\n')
    text = pattern_http.sub(' ', tweetdb['text'])
    t = {}
    t['words'] = pattern_words.findall(text)
    tweets.append(t)
    id = tweetdb.get('id', '')
    text = tweetdb.get('text', '')
    text = pattern_gt.sub('>', pattern_lt.sub('<', text))
    textstripped = pattern_strip.sub(' ', text)
    retweet = int(bool(search(pattern_retweet, text)))
    ...
    followers = tweetdb['user']['followers_count']
    followees = tweetdb['user']['friends_count']
    length = len(text)
    lengthnourl = len(text) - sum([len(t) for t in pattern_strip.findall(text)])
    englishness = tweetdb.get('englishness', 0)
    enlang = int(tweetdb.get('user', {}).get('lang') == 'en')
    valence = tweetdb.get('sentiment', 0)
    arousal = tweetdb.get('arousal', 0)
...
stopwords = [ line.strip() for line in urlopen(filename_stopwords) ]
stopwords.extend(['rt', 'lt', 'gt', 'amp'])
